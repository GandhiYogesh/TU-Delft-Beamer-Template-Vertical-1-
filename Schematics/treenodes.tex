...
\draw[->] (F)edge[bend left] node[midway, color=green, fill=white]{Yes}(B);
\draw[->] (F) edge[bend left] node[midway, color=red, fill=white]{No}(D);
\draw[->] (E) edge[bend right] node[midway, color=blue, fill=white]{FE analysis} (A);
\end{tikzpicture}
\end{document}